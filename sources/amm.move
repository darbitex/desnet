...
module desnet::amm {
    ...
    const SUPRA_FA_ADDR: address = @0xa;
    ...
    public(friend) fun create_pool_atomic(
        ...
        assert!(object::object_address(&supra_meta) == SUPRA_FA_ADDR, E_INVALID_FA_TYPE);
    ...
    public fun flash_borrow(
        ...
        let store = if (metadata_addr == SUPRA_FA_ADDR) {
            pool.supra_reserve
        } else {
    ...
    public fun flash_repay(
        ...
        let (reserve_store, fee_store, is_supra) = if (metadata_addr == SUPRA_FA_ADDR) {
    ...
    public fun pool_tokens(pool_addr: address): (Object<Metadata>, Object<Metadata>) acquires Pool {
        assert!(exists<Pool>(pool_addr), E_POOL_NOT_FOUND);
        let pool = borrow_global<Pool>(pool_addr);
        let supra_meta = object::address_to_object<Metadata>(SUPRA_FA_ADDR);
    ...
    public fun create_pool_atomic_for_test(
        ...
        create_pool_atomic(handle, supra_in, token_in, creator_pid)
    }
