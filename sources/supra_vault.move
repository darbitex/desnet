...
module desnet::supra_vault {
    ...
    /// H3 fix R3 (defense-in-depth): cap buyback amount per settle at 1% of pool SUPRA
    /// reserve. Bounds price impact → bounds attacker's pre-position profit envelope.
    /// Excess SUPRA redirects to PID owner (owner_amount = total_supra - capped_buyback).
    ...
    /// Buyback amount is capped at `MAX_BUYBACK_BPS_OF_RESERVE` (1%) of pool SUPRA
    ...
    public entry fun execute_settle(
        ...
        // H3 R3 defense-in-depth: cap buyback at 1% of pool SUPRA reserve.
        // Excess SUPRA redirects to PID owner. Bounds attacker pre-position profit
        // envelope (manipulation cost grows ~Δ², extractable profit grows ~Δ).
        let raw_buyback = total_supra / 2;
        let (supra_reserve, _token_reserve) = amm::reserves(vault.handle);
        let reserve_cap = (supra_reserve * MAX_BUYBACK_BPS_OF_RESERVE) / BPS_DENOM;
