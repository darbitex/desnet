# DeSNet v0.4.0-rc1 — Source Code

**Repo:** github.com/<org>/desnet (private during audit; full source provided here)
**Branch:** `opinion-pool-design`
**Source commit:** `6ace5a4` (paranoid audit fixes applied)

This file contains:
1. **Full source** of new module: `sources/opinion.move` (~1136 LoC including tests)
2. **Touchpoint excerpts** — only the changed/called signatures from sibling modules (`apt_vault.move`, `factory.move`, `history.move`, `profile.move`)

For full source of touchpoint modules (already audited at v0.3.3 R6 — commit `bf6d230`), reference the upstream repo at the same commit baseline.

---

## 1. `sources/opinion.move` (FULL)
...
module desnet::opinion {
    ...
    // Side / kind constant getters (for cross-module + frontend use)
...
## 2. Touchpoint excerpts (sibling module changes + called signatures)

### 2.1 `sources/apt_vault.move` — only friend grant added

```move
// Lines 22-28 (friend declarations, line 28 is the new addition)
use desnet::amm;

friend desnet::factory;
friend desnet::handle_fee_vault;
friend desnet::opinion;          // ← NEW for v0.4.0-rc1 (1 line)

// Lines 309-322 (existing burn_via_vault — UNCHANGED, called by opinion::burn_tax)
...
**Audit note**: Type-safe via FA framework. `fungible_asset::burn(&BurnRef, FA)` enforces FA metadata == BurnRef metadata at framework level. If opinion accidentally passes a non-creator-token FA, the abort happens deep in framework — no privilege escalation possible.

### 2.2 `sources/history.move` — VERB_OPINION + friend grant + view + assert relax

```move
// Lines 23-27 (friend list — last line is new)
friend desnet::mint;
friend desnet::pulse;
friend desnet::link;
friend desnet::press;
friend desnet::opinion;          // ← NEW

// Lines 31-41 (verb constants — VERB_OPINION is new)
...
const VERB_OPINION: u8 = 7;       // ← NEW

// Lines 108-117 (new_entry — assert bumped to allow VERB_OPINION)
public(friend) fun new_entry(
    ...
    assert!(verb <= VERB_OPINION, E_INVALID_VERB);   // ← was VERB_SYNC, now VERB_OPINION
    assert!(vector::length(&payload) <= MAX_PAYLOAD_BYTES, E_PAYLOAD_TOO_LARGE);
    Entry { verb, timestamp_secs, target, payload, asset }
}

// Lines 159-214 (append — UNCHANGED, used by opinion::create_opinion and emit_action)
public(friend) fun append(pid_addr: address, entry: Entry)
    acquires HistoryLog, HistoryChunk
{
    // ... existing body, lazy-init, chunk rotation, counter bumps
}

// Line 318 (verb_opinion view — new)
#[view]
public fun verb_opinion(): u8 { VERB_OPINION }   // ← NEW
```

**Audit note**: Per-verb counter logic in `append` falls through unchanged for VERB_OPINION (no new counter field — opinion module tracks its own counters in `PidOpinionMeta.opinion_count`). This is intentional compat-safe — adding a counter field would change `HistoryLog` struct layout (BREAKING upgrade).

### 2.3 `sources/profile.move` — only friend grant added

```move
// Lines 41-47 (friend list — last line is new)
friend desnet::mint;
friend desnet::link;
friend desnet::pulse;
friend desnet::press;
...
// Line 267 (derive_pid_address — UNCHANGED, public, used by opinion at create + emit_action)
public fun derive_pid_address(wallet: address): address {
    // pid_addr = create_object_address(@desnet, bcs(wallet))
}

// Line 684 (assert_pid_exists — UNCHANGED, public(friend), used by opinion::create_opinion)
public(friend) fun assert_pid_exists(pid_addr: address) {
    // aborts E_PROFILE_NOT_FOUND if no Profile resource at pid_addr
}

// Line 699 (derive_pid_signer — UNCHANGED, public(friend), used by opinion to derive
//                                pid_signer for create_named_object child of pid)
public(friend) fun derive_pid_signer(pid_addr: address): signer acquires Profile {
    // generate_signer_for_extending(&profile.extend_ref)
}

// Line 722 (profile_exists — UNCHANGED, public, used by opinion::emit_action)
public fun profile_exists(pid_addr: address): bool {
    exists<Profile>(pid_addr)
}
```

### 2.4 `sources/factory.move` — UNCHANGED (only public views called)

```move
// Line 491 — used by opinion::create_opinion to look up creator's $token
#[view]
...
// Line 515 — used by opinion::create_opinion as guest-restriction gate
#[view]
...
// Line 558 — used by opinion::burn_tax to find the apt_vault for tax burn
#[view]
...
**CRITICAL audit note (H1 fix verification)**: All three factory views above are keyed by **PID address**, NOT wallet address (per the existing factory.move docstring at line 474). Opinion's `create_opinion` was originally calling `owner_has_token(author_addr)` — wallet, not PID — which would cause every legitimate create to abort. **Fix applied at commit `6ace5a4`**: now passes `author_pid` consistently. See `04-tests-and-self-audit.md` H1 entry.

---

## 3. Out-of-scope file references (full source available in upstream repo)

These were NOT changed for v0.4.0-rc1 and were last audited at v0.3.3 R6 (commit `bf6d230`):

- `sources/factory.move` — full module, ~612 LoC. Provides `owner_has_token`, `token_metadata_of_owner`, `vault_addr_of_pid` views called by opinion.
- `sources/apt_vault.move` — full module, ~346 LoC (excluding the +1 friend line). Provides `burn_via_vault` friend delegation called by opinion.
- `sources/history.move` — full module, ~454 LoC + new VERB_OPINION (1 line) + new view (3 lines) + assert relax (1 line) + friend (1 line) = ~12 lines net. Provides `append`, `new_entry`, `verb_opinion`.
- `sources/profile.move` — full module, ~831 LoC + 1 friend line. Provides `derive_pid_address`, `derive_pid_signer`, `assert_pid_exists`, `profile_exists`.
- All other modules (mint, press, pulse, link, governance, amm, lp_*, reaction_emission, reference_gate, giveaway, handle_fee_vault, voter_history, assets) — UNCHANGED, NOT in scope.

---

## 4. Build + test commands (for reproducibility)
...
# Run all tests (93 total: 14 opinion + 79 other)
aptos move test

# Run opinion-only tests
aptos move test --filter opinion

# Expected output: 93/93 passed
```

Aptos CLI version used: per `aptos --version` at submission time (mainnet branch).
Move framework deps: `aptos-core` mainnet branch (as pinned in `Move.toml`).
